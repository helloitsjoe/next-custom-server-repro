...
2. **`AsyncLocalStorage` context is lost in the `app` or `pages` directories.** When we tried using an `AsyncLocalStorage` store as an alternative to `res.locals`, the store instance initialized in the custom server is undefined when accessing it in `getServerSideProps`.
